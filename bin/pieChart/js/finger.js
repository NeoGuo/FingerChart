...
finger = {
	...
	,setJSONData:function(chartType,container,obj,width,height,categoryField){
		var i=0;j=0;
		var labelArr=new Array();
		var fObj={};
		fObj.config={};
		if(obj.chart.xAxisName){fObj.config.compare=obj.chart.xAxisName;}
		if(categoryField==null||categoryField==""){fObj.config.categoryField="日期";}else{fObj.config.categoryField=categoryField;}
		if(obj.chart.caption!=null&&obj.chart.caption!=""){fObj.config.yTitle=obj.chart.caption;}else{fObj.config.yTitle="销售金额";}
		if(obj.chart.numbersuffix!=null&&obj.chart.numbersuffix!=""){fObj.config.qualifier=obj.chart.numbersuffix;}else{fObj.config.qualifier="元";}
		fObj.dataset=new Array();
		if(obj.categories!=null)
		{
			var categoriesLen=obj.categories.length;
			for (i = 0; i < categoriesLen; i++)
			{
				var category=obj.categories[i].category;
				var categorieLen=category.length;
				for(j=0;j<categorieLen;j++)
				{
					category[j].label=category[j].label.replace("\\n",",")
					if(labelArr[j]!=null)
					{
						labelArr[j]+=','+category[j].label;
					}
					else
					{
						labelArr[j]=category[j].label;
					...
					fObj.dataset[j]={};
					fObj.dataset[j][fObj.config.categoryField]=labelArr[j];
				}
			}
			var dataserLen=obj.dataset.length;
			for(i=0;i<dataserLen;i++)
			{
				var data=obj.dataset[i];
				for(j=0;j<data.data.length;j++)
				{
					//console.log(j)
					if(fObj.dataset[j]==null)
					{
						fObj.dataset[j]={};
					}
					fObj.dataset[j][data.seriesname]=data.data[j].value;
				}
				if(i!=0)
				{
					fObj.config.yField+=','+data.seriesname;
				}
				else
				{
					fObj.config.yField=data.seriesname;
		...
		{
			if(categoryField==null||categoryField==""){fObj.config.categoryField='彩种';}else{fObj.config.categoryField=categoryField;}
			fObj.config.yField='value';
			var dataLen=obj.data.length;
			for(i=0;i<dataLen;i++)
			{
				if(fObj.dataset[i]==null)
				{
					fObj.dataset[i]={};
				}
				fObj.dataset[i][fObj.config.categoryField]=obj.data[i].label;
				fObj.dataset[i][fObj.config.yField]=obj.data[i].value;
			}
		}
		
		if(chartType=="CompareLineChart")
		{
			var yFieldArr=fObj.config.yField.split(",");
			var yFieldNewArr=new Array();
			var yFieldLen=yFieldArr.length;
			for(i=0;i<yFieldLen;i++)
			{
				if(yFieldArr[i].indexOf("虚线")==-1)
				{
					
					for(j=0;j<yFieldLen;j++)
					{
						if(yFieldArr[j].indexOf("虚线")!=-1)
						{
							if(yFieldArr[j].replace("-虚线","")==yFieldArr[i])
							{
								yFieldNewArr.push(yFieldArr[i]);
								yFieldNewArr.push(yFieldArr[j]);
								continue;
							}
						}
					}
			...
			fObj.config.yField=yFieldNewArr.join(",");
		}
		var dataStr=finger.objToStr(fObj);
		//return dataStr;
		//console.log(dataStr);
		finger.output(chartType,container,dataStr,width,height);
	}
	/**
	* @todo：输出图形报表
	* @date：2008-7-3 16:38
	...
	*/
	,output: function(chartType,container,data,extendConfig,width,height) {
		...
		swfobject.embedSWF(swfURL,container,"100%" ,"100%", "9.0.0", "playerProductInstall.swf",configObj, this.params,this.attributes);
	}
	/**
