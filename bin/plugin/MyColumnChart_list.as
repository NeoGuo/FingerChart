package
{
	...
	public class MyColumnChart_list extends MovieClip implements IChartPlugin
	{
		...
		public function MyColumnChart_list()
		{
		...
		public function initPlugin(container:IPluginContainer):void
		{
			...
			var categoryField:String = dataset.config.categoryField;
			var allYField:Array = [];
			while (yField[j])
			{
				allYField[j] = 0;
			...
			var maxTextWidth1:int=0;
			var maxTextWidth2:int=0;
			var maxTextWidth3:int = 0;
			...
			for (i = 0; i < dataset.collection.length; i++)
			{
				...
				var str2:String = StringUtils.fomart(dataset.collection[i][yField[0]]) + danweiArr[0]+" ";
				var str3:String = '';
			...
			for (i = 0; i < dataset.collection.length; i++)
			{
				...
				btnLabel2.text = StringUtils.fomart(dataset.collection[i][yField[0]]) + danweiArr[0];
				btnLabel3.setPadding(TextFormatAlign.RIGHT);
				if (dataset.collection[i][yField[0] + "_tooltext"])
				{
					var tooltextStr:String =dataset.collection[i][yField[0] + "_tooltext"];
					btnLabel3.color = 0x5a5a5a;
					btnLabel3.text = tooltextStr;
				}
				else
				{
				...
				//(btn.txt as TextField).setTextFormat(new TextFormat(null, null, 0xFF0000),String(btn.txt.text).length-maxTextWidth2-maxTextWidth3, String(btn.txt.text).length);
