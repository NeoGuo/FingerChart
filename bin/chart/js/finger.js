...
finger = {
	...
	,setJSONData:function(chartType,container,obj,width,height,chartId,compare){
		...
		var fObj={};
		fObj.config={};
		if(obj.chart.showItemTotal){fObj.config.showItemTotal=obj.chart.showItemTotal;}else{fObj.config.showItemTotal="1";}
		if(obj.chart.xAxisName){fObj.config.categoryField=obj.chart.xAxisName;}else{fObj.config.categoryField="日期";}
		if(obj.chart.yAxisName!=null&&obj.chart.yAxisName!=""){fObj.config.yTitle=obj.chart.yAxisName;}else{fObj.config.yTitle=" ";}
		if(compare==null||compare==""){fObj.config.compare="虚线：同期 实线：当期";}
		if(obj.chart.numbersuffix!=null&&obj.chart.numbersuffix!=""){fObj.config.qualifier=obj.chart.numbersuffix;}else{fObj.config.qualifier=" ";}
		...
		if(obj.categories!=null)
		{
			...
			for(i=0;i<dataserLen;i++)
			{
                ...
                if(data.seriesname=="总值")
                {
                    fObj.config.showYField="总值";
                }
				if(i!=0)
				{
					fObj.config.yField+=','+data.seriesname;
					fObj.config.itemtotals+=','+data.itemtotal;
				}
				else
				{
					fObj.config.yField=data.seriesname;
		...
		{
			//if(categoryField==null||categoryField==""){fObj.config.categoryField=' ';}else{fObj.config.categoryField=categoryField;}
			...
			for(i=0;i<dataLen;i++)
			{
				...
				if(obj.data[i].tooltext!=null)
				{
					fObj.dataset[i][fObj.config.yField+"_tooltext"]=obj.data[i].tooltext;
				}
			}
		}
