package com.riameeting.finger.display.skin
{
	...
	public class AxisSkin extends MovieClip implements ISkin
	{
		...
		public function updateDisplayList(parms:Object=null):void
		{
			...
			//create label
			if(parent is ReversalAxis) {
				createLabelsByReversalMode();
			} else {
				createLabels();
			}
		}
		...
		protected function drawBottomContainer(w:Number,h:Number):void {
			...
			var colorAlpha1:Number = style["lineAlphas"][0]/100;
			var colorAlpha2:Number = style["lineAlphas"][1]/100;
			...
			if(allBottomLabelWidth > _hostComponent.graphicArea.width) {
				...
				for(i=1;i<_hostComponent.countX;i++) {
					if(count == hideIndex) {
						...
						labelContainer.getChildByName("l"+i).visible = false;
						count++;
					}
				}
			}
		...
		/**
		 * 创建需要显示的标签(用于BarChart)
		 * 
		 */	
		protected function createLabelsByReversalMode():void {
			var style:Object = _hostComponent.style;
			//bottom label
			var i:uint=0,currentLabel:Label;
			_hostComponent.realGridHeight = _hostComponent.graphicArea.width/_hostComponent.countY;
			_hostComponent.realGridWidth = (_hostComponent.graphicArea.height-int(style["offsetV"])*2)/(_hostComponent.countX-1);
			for(i=1;i<=_hostComponent.countY;i++) {
				currentLabel = ObjectFactory.produce(Label,"Label");
				currentLabel.color = style["bottomColor"];
				currentLabel.x = int(style["paddingTop"]) + _hostComponent.realGridHeight*i;
				currentLabel.y = _hostComponent.graphicArea.y + _hostComponent.graphicArea.height + 10;
				currentLabel.text = String(_hostComponent.maxValueY - int(((_hostComponent.maxValueY-_hostComponent.minValueY)/_hostComponent.countY)*(_hostComponent.countY-i)));
				labelContainer.addChild(currentLabel);
			}
			//left label
			for(i=0;i<_hostComponent.countX;i++) {
				...
				currentLabel.name = "l"+i;
				if(_hostComponent.xField != null) {
					currentLabel.text = String(int((_hostComponent.maxValueX/(_hostComponent.countX-1))*i));
				} else {
					currentLabel.text = _hostComponent.dataset.collection[i][_hostComponent.dataset.config["categoryField"]];
				}
				currentLabel.y = int(style["paddingTop"]) + _hostComponent.realGridWidth*(_hostComponent.countX-i-1) + int(style["offsetV"]) - currentLabel.height/2;
				labelContainer.addChild(currentLabel);
			}
		}
		/**
