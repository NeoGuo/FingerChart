package com.riameeting.finger.display.chart
{
	...
	public class AbstractChart extends Sprite implements IChart
	{
		...
		//style
		private var _style:Object = {bgColors:[0xFFFFFF,0xFFFFFF],bgAlphas:[100,100],lineColors:[0x000000,0x000000],lineAlphas:[0,5]};
		 ...
		 * 
		 */		
		public function get chartConfig():Object {return _chartConfig;}
		public function set chartConfig(value:Object):void {_chartConfig = value;}
		/**
		 * CSS样式定义
		 * @return 对象
		...
		{
			if(value == null) throw new Error("dataset not allow null");
			_dataset = value;
			_axis.dataset = value;
		 ...
		 * 
		 */	
		public function get axis():IAxis{return _axis;}
		public function set axis(value:IAxis):void{_axis = value;}
		/**
		 * 图表的4个显示层级之一：图形容器
		 * @return 图形容器
		 * 
		 */
		public function get chartGraphicContainer():IChartGraphicContainer{return _chartGraphicContainer;}
		public function set chartGraphicContainer(value:IChartGraphicContainer):void{_chartGraphicContainer = value;}
		/**
		 * 图表的4个显示层级之一：鼠标提示容器
		 * @return 鼠标提示容器
		 * 
		 */
		public function get tooltipContainer():ITooltipContainer{return _tooltipContainer;}
		public function set tooltipContainer(value:ITooltipContainer):void{_tooltipContainer = value;}
		/**
		 * 图表的4个显示层级之一：插件容器
		 * @return 插件容器
		 * 
		 */
		public function get pluginContainer():IPluginContainer{return _pluginContainer;}
		public function set pluginContainer(value:IPluginContainer):void{_pluginContainer = value;}
		/**
		 * CSS解析器
		 */		
		...
		public function AbstractChart(axis:IAxis, chartGraphicContainer:IChartGraphicContainer, tooltipContainer:ITooltipContainer, pluginContainer:IPluginContainer, width:uint, height:uint)
		{
			...
			if(Alert.container == null) Alert.container = this;
		}
		/**
		 * 图表宽度
		 * @return 宽度值
		 * 
		 */		
		override public function get width():Number {return chartWidth;}
		 ...
		 * 
		 */
		override public function get height():Number {return chartHeight;}
		...
		public function loadAssets(chartConfigObj:Object = null):void {
			if(chartConfigObj != null) chartConfig = chartConfigObj;
			var assets:Array = [];
			if(chartConfig.data != null) assets.push(chartConfig.data);
			if(chartConfig.skin != null) assets.push(chartConfig.skin);
			if(chartConfig.css != null) assets.push(chartConfig.css);
		...
		protected function loadAssetsCompleteHandler():void {
			...
			if(chartConfig.css != null) {
				...
				if(cssParser.getStyle("axis") != null) axis.style = ObjectUtils.mergePromps(cssParser.getStyle("axis"),axis.style);;
				if(cssParser.getStyle("gContainer") != null) chartGraphicContainer.style = ObjectUtils.mergePromps(cssParser.getStyle("gContainer"),chartGraphicContainer.style);
				if(cssParser.getStyle("tContainer") != null) tooltipContainer.style = ObjectUtils.mergePromps(cssParser.getStyle("tContainer"),tooltipContainer.style);
				if(cssParser.getStyle("pContainer") != null) pluginContainer.style = ObjectUtils.mergePromps(cssParser.getStyle("pContainer"),pluginContainer.style);
				if(_legend != null && cssParser.getStyle("legend") != null) _legend.style = ObjectUtils.mergePromps(cssParser.getStyle("legend"),_legend.style);
				if(cssParser.getStyle("global") != null) {
					...
					ChartGlobal.colorCollection = NumberTool.extendArrayByClone(ChartGlobal.colorCollection,10);
				}
			}
			//skin
		...
		protected function drawChart(e:Event = null):void {
			...
			if(e != null) ChartGlobal.skinSource = e.currentTarget.content;
			...
			if(_legend != null) _legend.dataset = dataset;
			contextMenuHandler();
		...
		protected function contextMenuHandler():void {
			...
			if(myContextMenu.customItems == null) return;
