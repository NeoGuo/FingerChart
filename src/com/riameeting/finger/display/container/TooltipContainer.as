package com.riameeting.finger.display.container
{
	...
	{
		protected var _dataset:DatasetVO;
		protected var _tooltipClass:Class;

		protected var tooltip:ITooltip;
		protected var axisRect:Rectangle;
		protected var graphicsCollection:Array;
		protected var _style:Object={color: 0x000000, tipbgColor: 0xFFFFFF, tipbgAlpha: 100, borderWidth: 3, borderColor: 0x000000, borderAlpha: 20};
		protected var _chartRef:IChart;
		/**
		 * 监测规则
		 */
		...
		public function set dataset(value:DatasetVO):void
		{
			...
			{
				(chartRef as DisplayObjectContainer).addEventListener(MouseEvent.MOUSE_DOWN, containerMouseHandler);
			}
			else
			{
		...
		protected function containerMouseHandler(e:MouseEvent):void
		{
			...
			mousePoint.x=mouseX;
			mousePoint.y = mouseY;
			//if (seriel.visible == false) { return; }
		...
		public function showToolTip(nearlyGraphic:IChartGraphic):void
		{
			tooltip.show(nearlyGraphic.data.tipString[nearlyGraphic.yField]);
			locateTooltip(nearlyGraphic.x + nearlyGraphic.center.x, nearlyGraphic.y + nearlyGraphic.center.y);
		}
