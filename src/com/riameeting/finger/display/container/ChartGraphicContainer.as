package com.riameeting.finger.display.container
{
	...
	import com.riameeting.utils.LabelResetTool;
	...
	public class ChartGraphicContainer extends Sprite implements IChartGraphicContainer
	{
		...
		private var _style:Object = {color:0x000000,benchmarkColor:0x999999,lineWidth:2,dotWidth:2,lineMode:"bezier",shadow:false};
		...
		public function ChartGraphicContainer(chartRef:IChart,graphicClass:Class,graphicKey:String,benchmarkClass:Class,differentColors:Boolean,showEffect:IEffect,hideEffect:IEffect)
		{
			...
			mouseChildren = false;
			mouseEnabled = false;
			//FIXME:因为禁用了鼠标交互，ITEM_CLICK事件需要使用其它机制抛出
		...
		public function updateDisplayList(width:uint,height:uint):void {
			clear();
			//testI++;
			//Debug.trace("执行次数测试",testI);
			//shadow
			if(style["shadow"] == true)
				this.filters = [new DropShadowFilter(4,45,0x000000,0.3)];
			...
			maskRect.graphics.drawRect(axisRect.x-5,axisRect.y,axisRect.width+10,axisRect.height+5);
			maskRect.graphics.endFill();
			//mask = maskRect;
			//chartRef.axis.skin.mask = maskRect;
		 ...
		 * @param	seriesName
		 */
		public function hideAndReDraw(seriesName:String):void
		{
			var yfieldArr:Array = String(chartRef.dataset.config.yField).split(",", 100);
			ArrayUtil.removeValueFromArray(yfieldArr, seriesName);
			chartRef.dataset.config.yField = yfieldArr.join(",");
			//chartRef.dataset = chartRef.dataset;
			//trace(chartRef.dataset.config.yField);
		 ...
		 * @param	seriesName
		 */
		public function showAndReDraw(seriesName:String):void
		{
			var yfieldArr:Array = String(chartRef.dataset.config.yField).split(",", 100);
			yfieldArr.push(seriesName);
			chartRef.dataset.config.yField = yfieldArr.join(",");
			//chartRef.dataset = chartRef.dataset;
			//chartRef.updateDisplayList();
		}
		...
		protected function effectComplete():void {
			...
			if (chartRef.dataset.config.labelArr != null)
			{
				LabelResetTool.reset(chartRef.dataset.config.labelArr[0], chartRef.dataset.config.labelArr[1], ChartGlobal.height);
			}
		}
		/**
		 * 执行清理动作
