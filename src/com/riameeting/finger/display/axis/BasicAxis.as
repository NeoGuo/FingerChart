package com.riameeting.finger.display.axis
{
	...
	public class BasicAxis extends Sprite implements IAxis
	{
		...
		private var _style:Object={gridHeight: 50, paddingLeft: 80, paddingTop: 40, paddingRight: 20, paddingBottom: 70, offsetV: 20, color: 0x000000, lineColors: [0x000000, 0xCCCCCC], lineAlphas: [100, 100], lineStrokes: [1,1]};
		...
		protected function calculateMaxValue():void
		{
			...
			if (minValueY !=0)
			{
				countY = graphicArea.height / Number(style["gridHeight"])+1;
				maxValueY = NumberTool.getMaxNumberY2(maxValueY,minValueY, countY);
			}
			else
			{
				countY = graphicArea.height / Number(style["gridHeight"]) +1;
			...
			if (dataset.config.qualifier=="%")
			{
				if (dataset.config.yaxismaxvalue != null)
				{
					maxValueY = dataset.config.yaxismaxvalue;
				}
				else
				{
					maxValueY = 100;
		...
		public function BasicAxis(chartRef:IChart, ignoreOffsetV:Boolean=false)
		{
			...
			mouseChildren=false;
			mouseEnabled=false;
			//reset default style via DPI
			_style["paddingLeft"] = DPIReset.getNumber(_style["paddingLeft"]);
			_style["paddingRight"] = DPIReset.getNumber(_style["paddingRight"]);
			_style["paddingTop"] = DPIReset.getNumber(_style["paddingTop"]);
			_style["paddingBottom"] = DPIReset.getNumber(_style["paddingBottom"]);
		}
		 ...
		 *
		 */
		public function getAngle(vo:Object, yField:String):Object
		{
			return null
		}
