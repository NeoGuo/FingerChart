			...
			if (prevIndex != -1)
			{
				...
				if (style["lineMode"] == "line")
				{
					if (prevObject.enabled)
					{
						...
						(parent as Sprite).graphics.beginGradientFill(GradientType.LINEAR, [ColorUtils.fadeColor(color, 0xFFFFFF, 0.2), color], [0.3, 0], [0x00, 0xFF], matix);
						(parent as Sprite).graphics.moveTo(x, y - style.lineWidth);
						...
						(parent as Sprite).graphics.lineTo(x, axisRectHeight);
						(parent as Sprite).graphics.lineTo(x, y - style.lineWidth);
