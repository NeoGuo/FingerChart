package com.riameeting.finger.display.graphic
{
	...
	public class LineGraphic extends Sprite implements IChartGraphic
	{
		...
		public function set color(value:uint):void
		{
			...
			{
				_color = ChartGlobal.colorDict[this.yField + "compare"];
			}
			if (ChartGlobal.colorDict[this.yField] == null)
			{
				_color = value;
				ChartGlobal.colorDict[this.yField] = _color;
			}
			else
			{
				_color = ChartGlobal.colorDict[this.yField];
			}
			skin.updateDisplayList({color: _color});
		}
		...
		public function locate(value:Point = null, offset:uint = 10):void
		{
			...
			if (style["lineMode"] == "line")
			{
				if (prevIndex != -1)
				{
					...
					if (prevObject.enabled)
					{
						...
						(parent as Sprite).graphics.lineStyle(6, 0xFFFFFF,1);
						(parent as Sprite).graphics.moveTo(x - 0.5, y);
						(parent as Sprite).graphics.lineTo(prevPoint.x, prevPoint.y);
						(parent as Sprite).graphics.lineStyle(style["lineWidth"], color,1);
						(parent as Sprite).graphics.moveTo(x - 0.5, y);
						(parent as Sprite).graphics.lineTo(prevPoint.x, prevPoint.y);
			...
			data["tipString"][_yField] = "<b><font color='#FFFFFF' size='14'>" + _yField + "</font></b><br/>";
			var categoryField:String = chartRef.dataset.config["categoryField"];
			if (String(data[categoryField]).search(',') != -1)
			{
				var indexNum:int = chartRef.dataset.config.yField.split(',', 10).indexOf(_yField);
				data["tipString"][_yField] += categoryField + "  " + String(data[categoryField]).split(',', 10)[indexNum] + "<br/>";
			}
			else
			{
			...
			if (yTitle == ''||yTitle==' ')
			{
				data["tipString"][_yField] +=data[_yField];
			}
			else
			{
